...
void compute(
        ...
        double &num_renards,
        bool debug = true,
        const double taux_attaque = 0.01,
        const double taux_croissance_lapins = 0.3,
        const double taux_croissance_renards = 0.008,
        const double taux_mortalite = 0.1,
        const int duree = 50
) {
...
int main() {
    ...
    for (double taux_attaque(taux_attaque_debut); taux_attaque <= taux_attaque_fin; taux_attaque += 0.01) {
        ...
        double num_renards(renards_i), num_lapins(lapins_i);
        bool lapins_decroient(false),
                renards_decroient(false);
