...
int main() {
  ...
  while (true) {
    ...
    switch (proc) {
      case kProcCvRGB2Gray:cvtColor(frame, res, COLOR_RGB2GRAY);
        break;
      case kProcCvThreshold:res = bgr2gray(frame);
        adaptiveThreshold(res, res, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 3, 2);
        break;
      case kProcHomeRGB2Gray:res = bgr2gray(frame);
        break;
      case kProcHomeRGB2SortedGray:res = bgr2sorted_gray(frame);
        break;
      case kProcHomeThreshold:res = binarize(frame);
        break;
      default:res = frame;
        break;
    }
